...
def augment_real_controlled(config, copies_per_image, output_subdir):
    paths = config['paths']
    ...
    logger.info(f"Аугментация ({output_subdir}): исходных={len(source_images)}, копий на каждое={copies_per_image}, всего={total_generate}")
    ...
    if actual < total_generate:
        ...
        logger.warning(f"Недобор {shortage}, добиваем...")
    ...
    logger.info(f"{output_subdir}: {final_count}/{total_generate}")
