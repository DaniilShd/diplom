...
from utils.dataset_utils import read_yolo_labels, write_yolo_labels, resize_image_and_labels, count_dataset_images

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)


def copy_real_dataset(config):
    paths = config['paths']
    ...
    for split in ['train', 'val', 'test']:
        ...
        if not src_split.exists():
            logger.warning(f"Сплит {split} не найден")
            continue
        ...
        for img_path in tqdm(src_images, desc=f"  {split}"):
            ...
            image_resized, bboxes, class_labels = resize_image_and_labels(image, bboxes, class_labels, target_size)
            
            cv2.imwrite(str(dst_split / 'images' / f"{img_path.stem}.jpg"), image_resized, [cv2.IMWRITE_JPEG_QUALITY, jpeg_quality])
            write_yolo_labels(dst_split / 'labels' / f"{img_path.stem}.txt", bboxes, class_labels)
