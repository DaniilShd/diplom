...
from utils.dataset_utils import read_yolo_labels, write_yolo_labels, resize_image_and_labels

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)


def copy_synthetic_dataset(config):
    paths = config['paths']
    ...
    for img_path in tqdm(src_images, desc="  Копирование"):
        ...
        cv2.imwrite(str(output_dir / 'images' / f"{img_path.stem}.jpg"), image_resized, [cv2.IMWRITE_JPEG_QUALITY, jpeg_quality])
        write_yolo_labels(output_dir / 'labels' / f"{img_path.stem}.txt", bboxes, class_labels)
