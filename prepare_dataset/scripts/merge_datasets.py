...
def merge_one_dataset(name, dataset_cfg, experiment_data, output_base, yolo_config):
    output_dir = output_base / name
    
    logger.info(f"Сборка {name}: {dataset_cfg.get('description', '')}")
    ...
    for source in dataset_cfg['train']:
        ...
        if not src_images.exists():
            logger.warning(f"Источник не найден: {src_images}")
            continue
        
        image_files = [f for f in src_images.glob('*') if f.suffix.lower() in ['.jpg', '.jpeg', '.png']]
...
def merge_all_datasets(config):
    ...
    for name, dataset_cfg in datasets_cfg.items():
        output_dir = merge_one_dataset(name, dataset_cfg, experiment_data, datasets_output, yolo_config)
    ...
    logger.info(f"Собрано датасетов: {len(results)}")
