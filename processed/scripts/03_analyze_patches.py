...
def plot_pie_patches_per_class(patches_per_class, names, rpt):
    ...
    axes[0].pie(sizes, labels=labels, autopct='%1.1f%%', colors=colors_pie, textprops={'fontsize': 11})
    ...
    for bar, v in zip(bars, sizes):
        axes[1].text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(sizes)*0.01, str(v), ha='center', fontweight='bold')
...
def plot_cooccurrence(patches_per_class, names, rpt):
    cls_ids = sorted(patches_per_class.keys())
    n = len(cls_ids)
    labels = [names[c] if c < len(names) else f"Cls_{c}" for c in cls_ids]
...
def main():
    ...
    ax.bar(range(len(cls_ids)), [class_counts[i] for i in cls_ids], color=colors_hex)
